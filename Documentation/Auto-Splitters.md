...
When the `parent` parameter is null or omitted, the setting# Auto Splitters
...
##### game
The currently connected [Process](https://msdn.microsoft.com/en-us/library/system.diagnostics.process(v=vs.110).aspx) object.
```
update { if (game.ProcessName == "snes9x") { } }
```
...
Using `settings.CurrentDefaultParent` can be useful when adding several settings with the same parent, without having to specify the parent everevery time
##### Tooltips
...
- Example: [Simple Autosplitter with Settings](https://raw.githubusercontent.com/tduva/LiveSplit-ASL/master/AlanWake.asl)
 will be added as top-level setting, unless `settings.CurrentDefaultParent` is set to something other than `null`:
